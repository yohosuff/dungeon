...
export class Renderer {
  ...
  setCanvas(canvas: HTMLCanvasElement) {
    ...
    this.primaryContext = canvas.getContext('2d') as CanvasRenderingContext2D;
    ...
    this.secondaryContext = this.secondaryCanvas.getContext('2d') as CanvasRenderingContext2D;
  }
  ...
  draw() {
    ...
    for(const tile of this._camera.visibleTiles.filter(tile => tile.inFOV)) {
      this.drawTile(context, tile.type === 0 ? 'wall' : 'floor', tile.position);
    }

    this._playerManager.sortOtherPlayersByY();

    for(const player of this._playerManager.getOtherPlayersAboveMe()) {
    ...
    for(const player of this._playerManager.getOtherPlayersBelowMe()) {
